...
- (void)viewDidLoad {
    ...
    NSString *performanceScript = @"(function(){"
      ...
      "sxPositionPullRefreshIndicator(document);new MutationObserver(function(rs){var changed=false;rs.forEach(function(r){Array.from(r.addedNodes||[]).forEach(function(n){if(n&&n.nodeType===1){changed=true;sxPositionPullRefreshIndicator(n);}});});if(changed)setTimeout(function(){sxPullRefreshSnapshot('mutation');},0);}).observe(document.documentElement,{childList:true,subtree:true});setTimeout(function(){sxPullRefreshSnapshot('initial-800ms');},800);document.addEventListener('touchend',function(){setTimeout(function(){sxPullRefreshSnapshot('touchend-0');},0);setTimeout(function(){sxPullRefreshSnapshot('touchend-250');},250);setTimeout(function(){sxPullRefreshSnapshot('touchend-800');},800);},{passive:true,capture:true});"
