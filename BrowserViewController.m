...
- (void)viewDidLoad {
    ...
    NSString *performanceScript = @"(function(){"
      ...
      "function probeMenuButtons(){if(menuProbeDone)return;if(!postSent){setTimeout(probeMenuButtons,250);return;}menuProbeDone=true;var rows=[].slice.call(document.querySelectorAll('button,[role=\\\"button\\\"]')).map(function(el,i){var r=el.getBoundingClientRect();return {i:i,tag:el.tagName,testid:el.getAttribute('data-testid')||'',aria:el.getAttribute('aria-label')||'',title:el.getAttribute('title')||'',x:Math.round(r.x),y:Math.round(r.y),w:Math.round(r.width),h:Math.round(r.height),visible:!!(r.width&&r.height)};}).filter(function(x){return x.visible&&x.y<220;}).slice(0,30);send('menu-button-probe',{count:rows.length,buttons:rows});}"
      "setTimeout(probeMenuButtons,100);"
