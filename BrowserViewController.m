...
- (void)viewDidLoad {
    ...
    NSString *performanceScript = @"(function(){"
      ...
      "document.addEventListener('touchstart',function(e){if(e.touches.length!==1)return;var t=e.touches[0];touch={startY:t.clientY,lastY:t.clientY,startTime:performance.now(),startScrollY:window.scrollY,startHeader:headerInfo(),maxDown:0,maxUp:0};if(window.scrollY<=120){send('pull-refresh-probe-start',{scrollY:Math.round(window.scrollY*100)/100,fingerY:Math.round(t.clientY),topNav:nodeSnapshot(findTopNavBar(),-1)});startHeaderTrace();sendHeaderLayout('top-zone-touchstart',true);}},{passive:true,capture:true});"
      ...
      "function hitElementSummary(el){if(!el)return null;var r=el.getBoundingClientRect();return {tag:el.tagName,id:el.id||'',testid:el.getAttribute('data-testid')||'',role:el.getAttribute('role')||'',text:(el.innerText||'').trim().slice(0,100),rect:{x:Math.round(r.x),y:Math.round(r.y),w:Math.round(r.width),h:Math.round(r.height)}};}"
      ...
      "function sendHeaderHitTest(trigger){var vv=window.visualViewport,w=window.innerWidth,h=window.innerHeight,x=Math.round(w/2),ys=[10,Math.round(h*0.2),Math.round(h*0.35),Math.round(h*0.5),Math.round(h*0.65),Math.round(h*0.8)];var nav=findTopNavBar(),nr=nav?nav.getBoundingClientRect():null;send('header-hit-test',{trigger:trigger,scrollY:Math.round(window.scrollY*100)/100,viewport:{w:w,h:h,offsetTop:vv?Math.round(vv.offsetTop*100)/100:null,pageTop:vv?Math.round(vv.pageTop*100)/100:null},topNav:nr?{x:Math.round(nr.x),y:Math.round(nr.y),w:Math.round(nr.width),h:Math.round(nr.height)}:null,points:ys.map(function(y){return {x:x,y:y,elements:hitTestStackAt(x,y)};})});}"
      ...
      "document.addEventListener('touchend',function(){if(!touch)return;var duration=Math.round(performance.now()-touch.startTime),fingerDelta=Math.round(touch.lastY-touch.startY),end=headerInfo(),wasTopPull=touch.startScrollY<=120&&touch.maxDown>=20;if(Math.abs(fingerDelta)>=8||touch.startHeader.visible!==end.visible){send('home-header-touch',{fingerDeltaY:fingerDelta,maxFingerDown:Math.round(touch.maxDown),maxFingerUp:Math.round(touch.maxUp),durationMs:duration,startScrollY:Math.round(touch.startScrollY),endScrollY:Math.round(window.scrollY),startHeaderVisible:touch.startHeader.visible===true,endHeaderVisible:end.visible===true,startHeaderY:touch.startHeader.y==null?null:touch.startHeader.y,endHeaderY:end.y==null?null:end.y});}if(wasTopPull){send('pull-refresh-touchend',{fingerDeltaY:fingerDelta,maxFingerDown:Math.round(touch.maxDown),durationMs:duration,scrollY:Math.round(window.scrollY*100)/100});sendHeaderHitTest('touchend');setTimeout(forcePageRecomposite,120);startHeaderTrace();[50,150,300,600,1000].forEach(function(ms){setTimeout(function(){var nav=findTopNavBar(),vv=window.visualViewport;send('pull-refresh-after-'+ms+'ms',{scrollY:Math.round(window.scrollY*100)/100,topNav:nodeSnapshot(nav,-1),viewportOffsetTop:vv?Math.round(vv.offsetTop*100)/100:null,viewportPageTop:vv?Math.round(vv.pageTop*100)/100:null,elementAtTop:(function(){var el=document.elementFromPoint(Math.round(window.innerWidth/2),10);return el?{tag:el.tagName,testid:el.getAttribute('data-testid')||'',role:el.getAttribute('role')||'',text:(el.innerText||'').slice(0,80)}:null;})()});sendHeaderHitTest('after-'+ms+'ms');},ms);});}if(touch.startScrollY<=120||window.scrollY<=120){sendHeaderLayout('top-zone-touchend',true);setTimeout(function(){sendHeaderLayout('top-zone-after-touch-150ms',true);},150);setTimeout(function(){sendHeaderLayout('top-zone-after-touch-400ms',true);},400);}touch=null;},{passive:true,capture:true});"
