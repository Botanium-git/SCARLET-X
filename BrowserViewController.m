...
- (void)viewDidLoad {
    ...
    NSString *performanceScript = @"(function(){"
      ...
      "function forcePageRecomposite(){if(!document.documentElement||!document.body)return;var old=document.getElementById('scarletx-recomposite-probe');if(old)old.remove();var layer=document.createElement('div');layer.id='scarletx-recomposite-probe';layer.setAttribute('aria-hidden','true');layer.style.cssText='position:fixed;inset:0;z-index:2147483647;pointer-events:none;opacity:0.001;transform:translate3d(0,0,0);will-change:transform;contain:strict;background:rgba(0,0,0,0.001)';document.body.appendChild(layer);void layer.offsetHeight;requestAnimationFrame(function(){layer.style.transform='translate3d(0,0.01px,0)';void layer.offsetHeight;requestAnimationFrame(function(){layer.remove();void document.documentElement.offsetHeight;send('page-recomposite-probe-applied',{scrollY:Math.round(window.scrollY*100)/100});});});}"
