...
- (void)viewDidLoad {
    ...
    NSString *performanceScript = @"(function(){"
      ...
      "var menuWarmupDone=false;"
      "function tryMenuWarmup(){if(menuWarmupDone)return;if(!postSent){setTimeout(tryMenuWarmup,250);return;}if(location.pathname!==\'/home\')return;var btn=document.querySelector(\'[data-testid=DashButton_ProfileIcon_Link]\');if(!btn){send(\'menu-warmup-missing\',{selector:\'DashButton_ProfileIcon_Link\'});return;}menuWarmupDone=true;send(\'menu-warmup-start\',{testid:btn.getAttribute(\'data-testid\')||\'\',aria:btn.getAttribute(\'aria-label\')||\'\'});var cover=document.createElement(\'div\');cover.id=\'scarletx-menu-warmup-cover\';cover.setAttribute(\'aria-hidden\',\'true\');cover.style.cssText=\'position:fixed;inset:0;z-index:2147483647;background:transparent;pointer-events:none;\';document.documentElement.appendChild(cover);btn.click();setTimeout(function(){document.body.click();requestAnimationFrame(function(){requestAnimationFrame(function(){var el=document.getElementById(\'scarletx-menu-warmup-cover\');if(el)el.remove();send(\'menu-warmup-complete\',{method:\'body-click\',visual:\'transparent-cover\'});});});},700);}"
      "setTimeout(tryMenuWarmup,100);"
