...
- (void)viewDidLoad {
    ...
    NSString *performanceScript = @"(function(){"
      ...
      "function forceTopNavRecomposite(){var nav=findTopNavBar();if(!nav)return;var originalTransform=nav.style.transform,originalWillChange=nav.style.willChange;nav.style.willChange='transform';nav.style.transform='translate3d(0,0.01px,0)';void nav.offsetHeight;requestAnimationFrame(function(){nav.style.transform=originalTransform;nav.style.willChange=originalWillChange;void nav.offsetHeight;send('topnav-recomposite-applied',{scrollY:Math.round(window.scrollY*100)/100});});}"
      "document.addEventListener('touchend',function(){if(!touch)return;var duration=Math.round(performance.now()-touch.startTime),fingerDelta=Math.round(touch.lastY-touch.startY),end=headerInfo(),wasTopPull=touch.startScrollY<=120&&touch.maxDown>=20;if(Math.abs(fingerDelta)>=8||touch.startHeader.visible!==end.visible){send('home-header-touch',{fingerDeltaY:fingerDelta,maxFingerDown:Math.round(touch.maxDown),maxFingerUp:Math.round(touch.maxUp),durationMs:duration,startScrollY:Math.round(touch.startScrollY),endScrollY:Math.round(window.scrollY),startHeaderVisible:touch.startHeader.visible===true,endHeaderVisible:end.visible===true,startHeaderY:touch.startHeader.y==null?null:touch.startHeader.y,endHeaderY:end.y==null?null:end.y});}if(wasTopPull){send('pull-refresh-touchend',{fingerDeltaY:fingerDelta,maxFingerDown:Math.round(touch.maxDown),durationMs:duration,scrollY:Math.round(window.scrollY*100)/100});setTimeout(forceTopNavRecomposite,50);startHeaderTrace();[50,150,300,600,1000].forEach(function(ms){setTimeout(function(){var nav=findTopNavBar(),vv=window.visualViewport;send('pull-refresh-after-'+ms+'ms',{scrollY:Math.round(window.scrollY*100)/100,topNav:nodeSnapshot(nav,-1),viewportOffsetTop:vv?Math.round(vv.offsetTop*100)/100:null,viewportPageTop:vv?Math.round(vv.pageTop*100)/100:null,elementAtTop:(function(){var el=document.elementFromPoint(Math.round(window.innerWidth/2),10);return el?{tag:el.tagName,testid:el.getAttribute('data-testid')||'',role:el.getAttribute('role')||'',text:(el.innerText||'').slice(0,80)}:null;})()});},ms);});}if(touch.startScrollY<=120||window.scrollY<=120){sendHeaderLayout('top-zone-touchend',true);setTimeout(function(){sendHeaderLayout('top-zone-after-touch-150ms',true);},150);setTimeout(function(){sendHeaderLayout('top-zone-after-touch-400ms',true);},400);}touch=null;},{passive:true,capture:true});"
