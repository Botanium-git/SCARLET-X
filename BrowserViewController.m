...
- (void)viewDidLoad {
    ...
    NSString *performanceScript = @"(function(){"
      ...
      "document.addEventListener('touchstart',function(e){if(e.touches.length!==1)return;var t=e.touches[0];touch={startY:t.clientY,lastY:t.clientY,startTime:performance.now(),startScrollY:window.scrollY,startHeader:headerInfo(),maxDown:0,maxUp:0,repairOnDrag:pullCancelArmed};if(pullCancelArmed){send('post-pull-next-touchstart',{scrollY:Math.round(window.scrollY*100)/100,fingerY:Math.round(t.clientY)});}if(window.scrollY<=120){send('pull-refresh-probe-start',{scrollY:Math.round(window.scrollY*100)/100,fingerY:Math.round(t.clientY),topNav:nodeSnapshot(findTopNavBar(),-1)});sendHeaderLayout('top-zone-touchstart',true);}},{passive:true,capture:true});"
      "document.addEventListener('touchmove',function(e){if(!touch||e.touches.length!==1)return;var y=e.touches[0].clientY;touch.lastY=y;var d=y-touch.startY;if(d>touch.maxDown)touch.maxDown=d;if(d<touch.maxUp)touch.maxUp=d;if(touch.repairOnDrag&&!touch.repairTriggered&&Math.abs(d)>=8){touch.repairTriggered=true;pullCancelArmed=false;send('post-pull-next-scroll-start',{fingerDeltaY:Math.round(d),scrollY:Math.round(window.scrollY*100)/100,direction:d<0?'finger-up':'finger-down'});sendHeaderHitTest('before-next-scroll-repair');forceXPortalLayerPulse();setTimeout(function(){sendHeaderHitTest('after-next-scroll-repair');},180);}if(touch.startScrollY<=120&&d>=20&&!touch.pullLogged){touch.pullLogged=true;send('pull-refresh-pull-detected',{fingerDeltaY:Math.round(d),scrollY:Math.round(window.scrollY*100)/100});}inspectHeaderTransition('touchmove');},{passive:true,capture:true});"
      "function nearestFixed(el){for(var n=el;n&&n!==document.documentElement;n=n.parentElement){if(getComputedStyle(n).position==='fixed')return n;}return null;}function nearestTestId(el){return el&&el.closest?el.closest('[data-testid]'):null;}function hitElementSummary(el){if(!el)return null;var r=el.getBoundingClientRect(),nav=findTopNavBar(),fp=nav?nav.parentElement:null,tid=nearestTestId(el),fx=nearestFixed(el);return {tag:el.tagName,id:el.id||'',testid:el.getAttribute('data-testid')||'',nearestTestid:tid?(tid.getAttribute('data-testid')||''):'',role:el.getAttribute('role')||'',text:(el.innerText||'').trim().slice(0,100),inTopNav:!!(nav&&nav.contains(el)),inTopNavParent:!!(fp&&fp.contains(el)),nearestFixed:fx?{tag:fx.tagName,testid:fx.getAttribute('data-testid')||'',rect:(function(){var q=fx.getBoundingClientRect();return {x:Math.round(q.x),y:Math.round(q.y),w:Math.round(q.width),h:Math.round(q.height)};})()}:null,rect:{x:Math.round(r.x),y:Math.round(r.y),w:Math.round(r.width),h:Math.round(r.height)}};}"
      "function hitTestStackAt(x,y){var els=document.elementsFromPoint?document.elementsFromPoint(x,y):[];return els.slice(0,12).map(hitElementSummary);}"
      "function sendHeaderHitTest(trigger){var vv=window.visualViewport,w=window.innerWidth,h=window.innerHeight,x=Math.round(w/2),ys=[10,Math.round(h*0.2),Math.round(h*0.35),Math.round(h*0.5),Math.round(h*0.65),Math.round(h*0.8)];var nav=findTopNavBar(),nr=nav?nav.getBoundingClientRect():null;send('header-hit-test',{trigger:trigger,scrollY:Math.round(window.scrollY*100)/100,viewport:{w:w,h:h,offsetTop:vv?Math.round(vv.offsetTop*100)/100:null,pageTop:vv?Math.round(vv.pageTop*100)/100:null},topNav:nr?{x:Math.round(nr.x),y:Math.round(nr.y),w:Math.round(nr.width),h:Math.round(nr.height)}:null,points:ys.map(function(y){return {x:x,y:y,elements:hitTestStackAt(x,y)};})});}var postPullUntil=0,postPullHitLast=0;window.addEventListener('scroll',function(){var now=performance.now();if(now>postPullUntil||now-postPullHitLast<200)return;postPullHitLast=now;sendHeaderHitTest('post-pull-scroll');},{passive:true});"
      "function forceXPortalLayerPulse(){var layers=document.getElementById('layers');if(!layers)return;var old=document.getElementById('scarletx-x-portal-probe');if(old)old.remove();var outer=document.createElement('div');outer.id='scarletx-x-portal-probe';outer.setAttribute('aria-hidden','true');outer.className='css-g5y9jx r-aqfbo4 r-zchlnj r-1d2f490 r-1xcajam r-12vffkv';outer.style.pointerEvents='none';var rel1=document.createElement('div');rel1.className='css-g5y9jx r-12vffkv';var rel2=document.createElement('div');rel2.className='css-g5y9jx r-12vffkv';var group=document.createElement('div');group.className='css-g5y9jx r-1ny4l3l';var rel3=document.createElement('div');rel3.className='css-g5y9jx r-16y2uox r-1wbh5a2';var fixed=document.createElement('div');fixed.className='css-g5y9jx r-1p0dtai r-1d2f490 r-1xcajam r-zchlnj r-ipm5af r-1j63xyz r-z2knda r-11z020y r-7qv4eb r-1habvwh';fixed.style.pointerEvents='none';var marker=document.createElement('div');marker.style.cssText='width:1px;height:1px;opacity:0;pointer-events:none';fixed.appendChild(marker);rel3.appendChild(fixed);group.appendChild(rel3);rel2.appendChild(group);rel1.appendChild(rel2);outer.appendChild(rel1);layers.appendChild(outer);void fixed.offsetHeight;var r=fixed.getBoundingClientRect();send('x-portal-probe-mounted',{scrollY:Math.round(window.scrollY*100)/100,rect:{x:Math.round(r.x),y:Math.round(r.y),w:Math.round(r.width),h:Math.round(r.height)}});requestAnimationFrame(function(){requestAnimationFrame(function(){setTimeout(function(){outer.remove();void layers.offsetHeight;send('x-portal-probe-removed',{scrollY:Math.round(window.scrollY*100)/100});},120);});});}"
      "document.addEventListener('touchend',function(){if(!touch)return;var duration=Math.round(performance.now()-touch.startTime),fingerDelta=Math.round(touch.lastY-touch.startY),end=headerInfo(),wasTopPull=touch.startScrollY<=120&&touch.maxDown>=20;if(Math.abs(fingerDelta)>=8||touch.startHeader.visible!==end.visible){send('home-header-touch',{fingerDeltaY:fingerDelta,maxFingerDown:Math.round(touch.maxDown),maxFingerUp:Math.round(touch.maxUp),durationMs:duration,startScrollY:Math.round(touch.startScrollY),endScrollY:Math.round(window.scrollY),startHeaderVisible:touch.startHeader.visible===true,endHeaderVisible:end.visible===true,startHeaderY:touch.startHeader.y==null?null:touch.startHeader.y,endHeaderY:end.y==null?null:end.y});}if(wasTopPull){send('pull-refresh-touchend',{fingerDeltaY:fingerDelta,maxFingerDown:Math.round(touch.maxDown),durationMs:duration,scrollY:Math.round(window.scrollY*100)/100});pullCancelArmed=true;postPullUntil=performance.now()+5000;postPullHitLast=0;send('post-pull-repair-armed',{scrollY:Math.round(window.scrollY*100)/100});sendHeaderHitTest('touchend');[50,150,300,600,1000].forEach(function(ms){setTimeout(function(){var nav=findTopNavBar(),vv=window.visualViewport;send('pull-refresh-after-'+ms+'ms',{scrollY:Math.round(window.scrollY*100)/100,topNav:nodeSnapshot(nav,-1),viewportOffsetTop:vv?Math.round(vv.offsetTop*100)/100:null,viewportPageTop:vv?Math.round(vv.pageTop*100)/100:null,elementAtTop:(function(){var el=document.elementFromPoint(Math.round(window.innerWidth/2),10);return el?{tag:el.tagName,testid:el.getAttribute('data-testid')||'',role:el.getAttribute('role')||'',text:(el.innerText||'').slice(0,80)}:null;})()});sendHeaderHitTest('after-'+ms+'ms');},ms);});}if(touch.startScrollY<=120||window.scrollY<=120){sendHeaderLayout('top-zone-touchend',true);setTimeout(function(){sendHeaderLayout('top-zone-after-touch-150ms',true);},150);setTimeout(function(){sendHeaderLayout('top-zone-after-touch-400ms',true);},400);}touch=null;},{passive:true,capture:true});"
      "document.addEventListener('touchcancel',function(){touch=null;},{passive:true,capture:true});"
      "var modalDiagTimer=0,modalDiagLast='';"
      "function styleSummary(el){if(!el)return null;var s=getComputedStyle(el),r=el.getBoundingClientRect();return {tag:el.tagName,id:el.id||'',testid:el.getAttribute('data-testid')||'',role:el.getAttribute('role')||'',ariaModal:el.getAttribute('aria-modal')||'',className:typeof el.className==='string'?el.className.slice(0,240):'',rect:{x:Math.round(r.x),y:Math.round(r.y),w:Math.round(r.width),h:Math.round(r.height)},css:{position:s.position,overflow:s.overflow,overflowY:s.overflowY,transform:s.transform,filter:s.filter,backdropFilter:s.backdropFilter,webkitBackdropFilter:s.webkitBackdropFilter,opacity:s.opacity,zIndex:s.zIndex},style:(el.getAttribute('style')||'').slice(0,300)};}"
      "function ancestorChain(el){var out=[],n=el;for(var i=0;i<12&&n;i++,n=n.parentElement)out.push(styleSummary(n));return out;}"
      "function layerChildrenSummary(){var root=document.getElementById('layers');if(!root)return {found:false};var kids=[].slice.call(root.children);return {found:true,root:styleSummary(root),children:kids.slice(-12).map(styleSummary)};}"
      "function modalLayerSnapshot(trigger){var dialogs=[].slice.call(document.querySelectorAll('[role=\\\"dialog\\\"],[aria-modal=\\\"true\\\"]')),menus=[].slice.call(document.querySelectorAll('[role=\\\"menu\\\"]')),target=dialogs.length?dialogs[dialogs.length-1]:(menus.length?menus[menus.length-1]:null),layers=document.getElementById('layers');send('modal-layer-state',{trigger:trigger,scrollY:Math.round(window.scrollY*100)/100,target:styleSummary(target),targetAncestors:ancestorChain(target),layers:layerChildrenSummary(),layersAncestors:ancestorChain(layers)});}"
      "function modalSnapshot(trigger){var dialogs=[].slice.call(document.querySelectorAll('[role=\\\"dialog\\\"],[aria-modal=\\\"true\\\"]'));var menus=[].slice.call(document.querySelectorAll('[role=\\\"menu\\\"]'));var html=document.documentElement,body=document.body,nav=findTopNavBar();var data={trigger:trigger,scrollY:Math.round(window.scrollY*100)/100,html:styleSummary(html),body:styleSummary(body),topNav:styleSummary(nav),dialogs:dialogs.slice(-3).map(styleSummary),menus:menus.slice(-3).map(styleSummary),dialogCount:dialogs.length,menuCount:menus.length};var key=JSON.stringify({hc:data.html&&data.html.className,hs:data.html&&data.html.style,bc:data.body&&data.body.className,bs:data.body&&data.body.style,dc:data.dialogCount,mc:data.menuCount});if(trigger==='mutation'&&key===modalDiagLast)return;modalDiagLast=key;send('modal-state',data);}"
      "function scheduleModalSnapshot(){clearTimeout(modalDiagTimer);modalDiagTimer=setTimeout(function(){modalSnapshot('mutation');var d=document.querySelector('[role=\\\"dialog\\\"],[aria-modal=\\\"true\\\"],[role=\\\"menu\\\"]');if(d)modalLayerSnapshot('mutation-with-modal');},25);}"
      "var modalObserver=new MutationObserver(scheduleModalSnapshot);modalObserver.observe(document.documentElement,{attributes:true,attributeFilter:['class','style','aria-hidden','aria-modal'],childList:true,subtree:true});"
      "document.addEventListener('click',function(e){var el=e.target&&e.target.closest?e.target.closest('[data-testid],button,[role=\\\"button\\\"],[role=\\\"menuitem\\\"]'):null;var label=el?((el.getAttribute('aria-label')||el.innerText||'').trim().slice(0,100)):'';if(/リポスト|Repost|引用|Quote|キャンセル|Cancel/.test(label)){send('modal-action-click',{label:label,testid:el.getAttribute('data-testid')||'',role:el.getAttribute('role')||''});[0,50,150,350,700].forEach(function(ms){setTimeout(function(){modalSnapshot('action-'+ms+'ms');modalLayerSnapshot('action-'+ms+'ms');},ms);});}},true);"
      "setTimeout(function(){modalSnapshot('initial');},500);"
    "})();";
