    ...
    NSString *settingsScript = @"(function(){"
        ...
        "function add(){"
          ...
          "while((node=walker.nextNode())){"
            "if(node.nodeValue&&node.nodeValue.indexOf('設定とプライバシー')!==-1){node.nodeValue=node.nodeValue.replace('設定とプライバシー','Scarlet X 設定');changed=true;break;}"
          "}"
          "if(!changed){"
            "var texts=[].slice.call(item.querySelectorAll('span'));"
            "var target=texts.find(function(e){return (e.innerText||'').trim().length>0;});"
            "if(target)target.textContent='Scarlet X 設定';"
          "}"
