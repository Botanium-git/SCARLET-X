...
- (void)viewDidLoad {
    ...
    NSString *performanceScript = @"(function(){"
      ...
      "var displayDOMSeen={};"
      "function collectDisplayDOM(root){var out=[];function add(el){if(!el||out.length>=120)return;var text=(el.innerText||el.getAttribute('aria-label')||'').replace(/\\s+/g,' ').trim();var href=el.getAttribute('href')||'';var testid=el.getAttribute('data-testid')||'';var aria=el.getAttribute('aria-label')||'';var role=el.getAttribute('role')||'';var hay=(text+' '+href+' '+testid+' '+aria).toLowerCase();if(!(hay.indexOf('grok')>=0||hay.indexOf('おすすめ')>=0||hay.indexOf('フォロー中')>=0||hay.indexOf('アプリ')>=0||hay.indexOf('ダウンロード')>=0||hay.indexOf('購入')>=0||hay.indexOf('download')>=0||hay.indexOf('purchase')>=0||hay.indexOf('following')>=0||hay.indexOf('for you')>=0))return;var key=el.tagName+'|'+href+'|'+testid+'|'+aria+'|'+text.slice(0,100);if(displayDOMSeen[key])return;displayDOMSeen[key]=1;out.push({tag:el.tagName||'',href:href,testid:testid,ariaLabel:aria,role:role,text:text.slice(0,160)});}if(root&&root.matches&&root.matches('a,button,[role=tab],[role=button]'))add(root);if(root&&root.querySelectorAll)Array.from(root.querySelectorAll('a,button,[role=tab],[role=button]')).forEach(add);if(out.length)send('display-dom-inventory',{pathname:location.pathname,count:out.length,items:out});}"
