...
- (void)viewDidLoad {
    ...
    NSString *performanceScript = @"(function(){"
      ...
      "function inspect(){"
        ...
        "if(!postSent){var post=document.querySelector('article[data-testid=\\\"tweet\\\"],article');if(post){var r=post.getBoundingClientRect(),text=(post.innerText||'').trim();if(r.height>40&&text.length>0){postSent=true;send('first-post-visible',{y:Math.round(r.y),h:Math.round(r.height),textLength:text.length});}}}"
