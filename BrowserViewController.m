...
- (void)viewDidLoad {
    ...
    NSString *performanceScript = @"(function(){"
      ...
      "setTimeout(runMenuInitProbe,100);"
      "var menuTapBaseline=null,menuTapAt=0,menuRequestSeq=0;"
      ...
      "var menuDataPrewarmDone=false,menuABEnabled=(Math.floor(Date.now()/1000)%2===0);"
      "send('menu-ab-assignment',{prewarm:menuABEnabled,method:'launch-parity'});"
      "function tryMenuDataPrewarm(){if(!menuABEnabled||menuDataPrewarmDone||!postSent||location.pathname!=='/home')return;if(!sxCommonHeaders.authorization||!sxCommonHeaders['x-csrf-token']){setTimeout(tryMenuDataPrewarm,250);return;}menuDataPrewarmDone=true;var url='/i/api/graphql/iWqvCFQkkYoobdIjmXPKoA/DelegatedAccountListQuery?variables=%7B%7D';var x=new XMLHttpRequest();x.open('GET',url,true);Object.keys(sxCommonHeaders).forEach(function(k){try{x.setRequestHeader(k,sxCommonHeaders[k]);}catch(e){}});x.addEventListener('loadend',function(){send('menu-data-prewarm',{status:x.status,operation:'DelegatedAccountListQuery'});});x.send();send('menu-data-prewarm-start',{operation:'DelegatedAccountListQuery'});}"
      "setTimeout(function waitPrewarm(){if(!menuABEnabled)return;if(postSent)tryMenuDataPrewarm();else setTimeout(waitPrewarm,250);},100);"
