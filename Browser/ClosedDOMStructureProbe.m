...
- (void)sx_probe_webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        NSString *script = @"(function(){function c(s){try{return document.querySelectorAll(s).length}catch(e){return -1}}function pathKind(h){if(!h)return 'empty';try{var u=new URL(h,location.href);if(u.origin!==location.origin)return 'external';var p=u.pathname||'/';if(p==='/'||p==='/home')return 'home';if(/^\\/[A-Za-z0-9_]{1,15}\\/?$/.test(p))return 'single-segment';if(p.indexOf('/i/')===0)return 'internal-i';if(p.indexOf('/settings')===0)return 'settings';if(p.indexOf('/messages')===0)return 'messages';if(p.indexOf('/notifications')===0)return 'notifications';if(p.indexOf('/search')===0||p==='/explore')return 'discovery';return 'other';}catch(e){return 'invalid'}}var p=document.querySelector('[data-testid=\\\"DashButton_ProfileIcon_Link\\\"]');var links=Array.from(document.querySelectorAll('a[href]'));var kinds={};links.forEach(function(a){var k=pathKind(a.getAttribute('href')||'');kinds[k]=(kinds[k]||0)+1;});var near=[];if(p){var root=p.parentElement&&p.parentElement.parentElement?p.parentElement.parentElement:p.parentElement;Array.from((root||document).querySelectorAll('a[href]')).slice(0,20).forEach(function(a){near.push(pathKind(a.getAttribute('href')||''));});}return {profileButton:!!p,profileImage:!!(p&&p.querySelector('img')),profileAriaLabel:!!(p&&p.hasAttribute('aria-label')),profileHref:!!(p&&p.hasAttribute('href')),profileExpanded:p?(p.getAttribute('aria-expanded')||''):null,dialogs:c('[role=\\\"dialog\\\"]'),menus:c('[role=\\\"menu\\\"]'),accountSwitcher:c('[data-testid=\\\"SideNav_AccountSwitcher_Button\\\"]'),accountSwitchButtons:c('button[aria-label$=\\\"に切り替える\\\"]'),navLinks:c('nav a[href]'),allLinks:links.length,linkKinds:kinds,nearProfileLinkKinds:near,images:c('img')};})()";
